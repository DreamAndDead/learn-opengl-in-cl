  ...
  (sdl2:with-init (:everything)
    ...
    (sdl2:with-window (win :w *width* :h *height* :flags '(:shown :opengl))
      ...
      (sdl2:with-gl-context (gl-context win)
        ...
        (setf *shader* (make-shader "getting-started/transformations/shader.vert"
                                    "getting-started/transformations/shader.frag"))
        ...
        (progn
          ...
          (let ((image (sdl2-image:load-jpg-rw "./getting-started/transformations/wall.jpg")))
        ...
        (progn
          ...
          (let ((image (sdl2-image:load-png-rw "./getting-started/transformations/awesomeface.png")))
            (gl:tex-image-2d :texture-2d 0 :rgb
                             (sdl2:surface-width image) (sdl2:surface-height image)
        ...
        (sdl2:with-event-loop (:method :poll)
          (:idle ()
                 ...
                 (use *shader*)
                 (let ((trans (sb-cga:identity-matrix))
                       (time (float (/ (sdl2:get-ticks) 1000))))
                   (setf trans (sb-cga:matrix* (sb-cga:translate* 0.5 -0.5 0.0)
                                               (sb-cga:rotate-around (sb-cga:vec 0.0 0.0 1.0) time)))
                   (set-mat4fv *shader* "Trans" trans))
