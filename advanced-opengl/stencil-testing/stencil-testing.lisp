...
(defun draw ()
  ...
  (let ((model (kit.glm:identity-matrix))
    ...
    (setf view (get-view-mat))
    (setf proj (kit.glm:perspective-matrix (kit.glm:deg-to-rad (fov *camera*)) (float (/ *width* *height*)) 0.1 100.0))
    (set-mat4fv *shader-pure-color* "model" model)
    (set-mat4fv *shader-pure-color* "view" view)
    (set-mat4fv *shader-pure-color* "proj" proj))
  ...
  (let ((model (kit.glm:scale* 1.1 1.1 1.1)))
    (set-mat4fv *shader-pure-color* "model" model))
  (draw-model *model* *shader-pure-color*)

  (let ((model (kit.glm:matrix* (kit.glm:translate* 0.0 0.0 -3.0) (kit.glm:scale* 1.1 1.1 1.1))))
    (set-mat4fv *shader-pure-color* "model" model))
  (draw-model *model* *shader-pure-color*))
  ...
  (sdl2:with-init (:everything)
    ...
    (sdl2:with-window (win :w *width* :h *height* :flags '(:shown :opengl))
      ...
      (sdl2:with-gl-context (gl-context win)
        ...
        (setf *shader* (make-shader "advanced-opengl/stencil-testing/shader.vert"
                                    "advanced-opengl/stencil-testing/shader.frag"))
        (setf *shader-pure-color* (make-shader "advanced-opengl/stencil-testing/pure-color.vert"
                                               "advanced-opengl/stencil-testing/pure-color.frag"))
