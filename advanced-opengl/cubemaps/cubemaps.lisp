...
(defun draw ()
  ...
  (set-uniformi *shader-quad* "framebuffer" 0)
  (gl:active-texture 0)
  (gl:bind-texture :texture-2d *frame-texture*)

  ;; FIXME image flip left/right 
  (draw-model *model-quad* *shader-quad*))
  ...
  (sdl2:with-init (:everything)
    ...
    (sdl2:with-window (win :w *width* :h *height* :flags '(:shown :opengl))
      ...
      (sdl2:with-gl-context (gl-context win)
        ...
        (setf *shader* (make-shader "advanced-opengl/cubemaps/shader.vert"
                                    "advanced-opengl/cubemaps/shader.frag"))
        (setf *shader-quad* (make-shader "advanced-opengl/cubemaps/quad.vert"
                                         "advanced-opengl/cubemaps/quad.frag"))
        (setf *shader-skybox* (make-shader "advanced-opengl/cubemaps/skybox.vert"
                                         "advanced-opengl/cubemaps/skybox.frag"))
        ...
        (setf *model-quad* (load-model "assets/box/quad.obj"))

        
        (gl:viewport 0 0 *width* *height*)

        (setf *cubemap-texture* (gl:gen-texture))
        (gl:bind-texture :texture-cube-map *cubemap-texture*)
        
        (macrolet ((load-cube-side (side file)
                     `(let ((image (sdl2-image:load-jpg-rw ,file)))
                       (gl:tex-image-2d ,side 0 :rgb
                                        (sdl2:surface-width image) (sdl2:surface-height image)
                                        0 :rgb :unsigned-byte (sdl2:surface-pixels image))
                       (sdl2:free-surface image))
                     ))
          (load-cube-side :texture-cube-map-positive-x "assets/skybox/right.jpg")
          (load-cube-side :texture-cube-map-negative-x "assets/skybox/left.jpg")
          (load-cube-side :texture-cube-map-positive-y "assets/skybox/top.jpg")
          (load-cube-side :texture-cube-map-negative-y "assets/skybox/bottom.jpg")
          (load-cube-side :texture-cube-map-positive-z "assets/skybox/front.jpg")
          (load-cube-side :texture-cube-map-negative-z "assets/skybox/back.jpg"))
        ...
        (setf *frame-buffer* (gl:gen-framebuffer))
        (gl:bind-framebuffer :framebuffer *frame-buffer*)

        (setf *frame-texture* (gl:gen-texture))
        (gl:bind-texture :texture-2d *frame-texture*)
        ;; (gl:tex-image-2d :texture-2d 0 :rgb *width* *height* 0 :rgb :unsigned-byte nil)
        (gl:tex-image-2d :texture-2d 0 :rgba *width* *height* 0 :rgba :unsigned-byte nil)
        (gl:tex-parameter :texture-2d :texture-min-filter :linear)
        (gl:tex-parameter :texture-2d :texture-mag-filter :linear)
        (gl:bind-texture :texture-2d 0)
        (gl:framebuffer-texture-2d :framebuffer :color-attachment0 :texture-2d *frame-texture* 0)
        ...
        (let ((check (gl:check-framebuffer-status :framebuffer)))
          (format t "Framebuffer status ~a~%" check))

        (let ((check (gl:check-named-framebuffer-status *frame-buffer* :framebuffer)))
          (format t "Framebuffer status ~a~%" check))
