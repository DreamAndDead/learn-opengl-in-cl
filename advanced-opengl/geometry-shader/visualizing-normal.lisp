...
(defun draw ()
  ...
  (set-uniformf *shader* "time" 0)

  (draw-model *model* *shader*)


  (use *shader-vis-normal*)
  ...
  (let ((model (kit.glm:identity-matrix))
    ...
    (setf view (get-view-mat))
    (setf proj (kit.glm:perspective-matrix (kit.glm:deg-to-rad (fov *camera*)) (float (/ *width* *height*)) 0.1 100.0))
    (set-mat4fv *shader-vis-normal* "model" model)
    (set-mat4fv *shader-vis-normal* "view" view)
    (set-mat4fv *shader-vis-normal* "proj" proj))

  (draw-model *model* *shader-vis-normal*))
    ...
    (sdl2:with-window (win :w *width* :h *height* :flags '(:shown :opengl))
      ...
      (sdl2:with-gl-context (gl-context win)
        ...
        (setf *shader* (make-shader "advanced-opengl/geometry-shader/shader.vert"
                                    ...
                                    "advanced-opengl/geometry-shader/shader.geom"))
        (setf *shader-vis-normal* (make-shader "advanced-opengl/geometry-shader/vis-normal.vert"
                                               "advanced-opengl/geometry-shader/vis-normal.frag"
                                               "advanced-opengl/geometry-shader/vis-normal.geom"))
